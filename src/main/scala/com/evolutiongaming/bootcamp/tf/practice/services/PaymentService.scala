...
object PaymentService {
  ...
  def apply[F[_]: FromFuture](paymentClient: PaymentClient): PaymentService[F] =
    (payment: Payment) => FromFuture[F].apply { paymentClient.process(token, payment) }
  ...
  implicit class PaymentServiceOps[F[_]](self: PaymentService[F]) {
    ...
    def toUnsafe(implicit toFuture: ToFuture[F]): PaymentService[Future] =
      new PaymentService[Future] {
        override def process(payment: Payment): Future[PaymentId] = toFuture(self.process(payment))
      }
  }
}
