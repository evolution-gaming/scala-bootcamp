...
trait IterableFactory[+CC[_]] extends Serializable {
  ...
  def tabulate[A](n1: Int, n2: Int, n3: Int, n4: Int)(
    f: (Int, Int, Int, Int) => A
  ): CC[CC[CC[CC[A]]] @uncheckedVariance] =
...
trait SeqFactory[+CC[A] <: SeqOps[A, Seq, Seq[A]]] extends IterableFactory[CC] {
  ...
  final def unapplySeq[A](x: CC[A] @uncheckedVariance): UnapplySeqWrapper[A] = new UnapplySeqWrapper(
    x
  ) // TODO is uncheckedVariance sound here?
}
...
object EvidenceIterableFactory {
  ...
  @SerialVersionUID(3L)
  private[this] class ToFactory[Ev[_], A: Ev, CC[_]](factory: EvidenceIterableFactory[CC, Ev])
  ...
  private class EvidenceIterableFactoryToBuildFrom[Ev[_], A: Ev, CC[_]](factory: EvidenceIterableFactory[CC, Ev])
...
trait ClassTagIterableFactory[+CC[_]] extends EvidenceIterableFactory[CC, ClassTag] {
  ...
  /** Produces a $coll containing a sequence of increasing of integers.
    ...
    *  @return  a $coll with values `start, start + 1, ..., end - 1`
    */
  def range[A: Integral: ClassTag](start: A, end: A): CC[A] = from(
    NumericRange(start, end, implicitly[Integral[A]].one)
  )
  ...
  /** Produces a three-dimensional $coll containing the results of some element computation a number of times.
    ...
    *  @return  A $coll that contains the results of `n1 x n2 x n3` evaluations of `elem`.
    */
  def fill[A: ClassTag](n1: Int, n2: Int, n3: Int)(elem: => A): CC[CC[CC[A]] @uncheckedVariance] =
...
object ClassTagIterableFactory {
  ...
  class AnyIterableDelegate[CC[_]](delegate: ClassTagIterableFactory[CC]) extends IterableFactory[CC] {
    ...
    override def range[A](start: A, end: A, step: A)(implicit i: Integral[A]): CC[A] =
      delegate.range[A](start, end, step)(i, ClassTag.Any.asInstanceOf[ClassTag[A]])
...
object SortedMapFactory {
  ...
  @SerialVersionUID(3L)
  private[this] class ToFactory[K: Ordering, V, CC[_, _]](factory: SortedMapFactory[CC])
      extends Factory[(K, V), CC[K, V]]
      with Serializable {
    def fromSpecific(it: IterableOnce[(K, V)]): CC[K, V] = factory.from[K, V](it)
    def newBuilder: Builder[(K, V), CC[K, V]]            = factory.newBuilder[K, V]
  ...
  private class SortedMapFactoryToBuildFrom[K: Ordering, V, CC[_, _]](factory: SortedMapFactory[CC])
